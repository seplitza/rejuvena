...
import ms from './replacers/ms';
import vWOrVh from './replacers/vw-vh';

export default class Value {
  value: any;
  outValue: any;
  prop: string;
  varsArr: Array<any> | null;
  stack: Array<any>;
  isOperation: boolean;
  /**
   * Constructor
   *
   * @param {*} value
   * @param {String} prop property for which value is calculated
   ...
   * @param {String} str
   */
  tryActions(actions: Array<any>, str: string) {
   ...
   * Tries calc ms
   */
  tryCalcMs(str: string) {
    if (ms.isMs(str)) {
      let factor = vars.get('$msFactor', this.varsArr);
      return ms.calc(str, factor);
    } else if (ms.isS(str)) {
      return ms.calcS(str);
    } else if (ms.isVs(str)) {
      return ms.calcVs(str);
    } else {
      return null;
    }
  }
   ...
   * Tries calc vw
   */
  tryCalcVw(str: string) {
    if (vWOrVh.isVw(str)) {
      return vWOrVh.calc(str);
    } else {
      return null;
    }
  }
